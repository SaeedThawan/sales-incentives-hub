    ...
    try {
      ...
      if (data && data.status === 'success') {
        if (data.generalRules) {
          ...
          setKitchenGeneralRules(data.generalRules);
        }
        if (data.groupRules && data.groupRules.length > 0) {
          setGroupRules(data.groupRules);
          setKitchenGroupRules(data.groupRules);
        }
        ...
        {currentUser.role !== 'rep' && (
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3 mb-6">
            ...
            <div className="bg-slate-800 border border-slate-700 p-3.5 rounded-2xl">
              ...
              <span className={`text-base font-extrabold font-mono ${companyTotals.overallGenPct >= activeGeneralRules.generalThresholdPct ? 'text-emerald-400' : 'text-amber-400'}`}>
            ...
            <div className="bg-slate-800 p-6 rounded-2xl border border-slate-700 shadow-xl space-y-6">
                        ...
                        <tr key={idx} className="hover:bg-slate-800/60">
                          ...
                          <td className="p-3">
                            <input
                              type="number"
                              disabled={currentUser.role !== 'manager' || monthStatus === 'approved'}
                              value={grpRule.thresholdPct}
                              onChange={(e) => updateOfficialGroup(idx, 'thresholdPct', parseFloat(e.target.value) || 0)}
                              className="w-20 bg-slate-800 border border-slate-700 rounded p-1.5 text-center text-teal-300 font-bold disabled:opacity-60"
                            />
                          </td>
                          <td className="p-3 font-sans">
                            <select
                              disabled={currentUser.role !== 'manager' || monthStatus === 'approved'}
                              value={grpRule.commType}
                              onChange={(e) => updateOfficialGroup(idx, 'commType', e.target.value)}
                              className="bg-slate-800 border border-slate-700 text-slate-200 rounded p-1.5 text-xs disabled:opacity-60"
                            >
                          ...
                          <td className="p-3">
                            <input
                              ...
                              onChange={(e) => updateOfficialGroup(idx, 'commValue', parseFloat(e.target.value) || 0)}
                              className="w-24 bg-slate-800 border border-slate-700 rounded p-1.5 text-center text-emerald-400 font-bold disabled:opacity-60"
                            />
                          </td>
                        </tr>
                      ))}
              ...
              <div className="space-y-3 pt-4 border-t border-slate-700">
                <h3 className="text-sm font-bold text-blue-400 flex items-center gap-2">
                  <i className="fa-solid fa-hand-holding-dollar"></i> شرائح عمولات التحصيل من إجمالي التحصيل المحقق
                </h3>
                <div className="overflow-x-auto border border-slate-700 rounded-xl">
                  <table className="w-full text-xs text-right bg-slate-900">
                    <thead className="bg-slate-950 text-slate-300 border-b border-slate-700">
                      <tr>
                        <th className="p-3">الشريحة</th>
                        <th className="p-3">من نسبة تحصيل (%)</th>
                        <th className="p-3">إلى نسبة تحصيل (%)</th>
                        <th className="p-3">نسبة العمولة (%)</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-800 font-mono">
                      {(generalRules.collectionTiers || []).map((tier, idx) => (
                        <tr key={idx} className="hover:bg-slate-800/60">
                          <td className="p-3 font-sans font-bold text-slate-400">شريحة {idx + 1}</td>
                          <td className="p-3">{tier.minPct}%</td>
                          <td className="p-3">{tier.maxPct}%</td>
                          <td className="p-3 font-bold text-blue-300 font-mono">{(tier.rate * 100).toFixed(2)}%</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              <div className="bg-slate-900 p-4 rounded-xl border border-slate-700">
                ...
                <input
                  ...
                  onChange={(e) => setKitchenGeneralRules({ ...kitchenGeneralRules, generalThresholdPct: parseFloat(e.target.value) || 0 })}
              ...
              <div className="bg-slate-900 p-4 rounded-xl border border-slate-700">
                ...
                <input
                  ...
                  onChange={(e) => setKitchenGeneralRules({ ...kitchenGeneralRules, generalTargetCommValue: parseFloat(e.target.value) || 0 })}
                  ...
                  <div className="grid grid-cols-2 gap-2 text-xs">
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => updateKitchenGroup(idx, 'thresholdPct', parseFloat(e.target.value) || 0)}
                    ...
                    <div>
                      ...
                      <input
                        type="number"
                        value={grp.commValue}
                        onChange={(e) => updateKitchenGroup(idx, 'commValue', parseFloat(e.target.value) || 0)}
