...
function App() {
    ...
    try {
      ...
      if (data && data.status === 'success') {
        if (data.generalRules) {
          ...
          if (!kitchenGeneralRules) setKitchenGeneralRules(JSON.parse(JSON.stringify(data.generalRules)));
        }
  ...
  const handleAddRepToGroup = (gIdx) => {
    ...
    if (!repId) {
      showToast('يرجى اختيار مندوب من القائمة أولاً');
      return;
    }
    updateKitchenRepGroupTarget(repId, gIdx, 10000);
    setSelectedRepToAdd(prev => ({ ...prev, [gIdx]: '' }));
    showToast('تم إشراك المندوب في المجموعة بنجاح');
  };
